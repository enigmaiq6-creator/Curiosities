...
class SubtitleEngine:
    ...
    def create_ass_subtitles(self, scene_data: List[Dict[str, Any]], output_ass_path: Path, total_video_duration: float = 60.0):
        """
        Genera subtítulos estilo DOCUMENTAL CLÁSICO con ALINEACIÓN ACÚSTICA PERFECTA:
        1. Posición elevada (MarginV=480 en vertical) para no tapar el nombre de la página ni la UI de Reels.
        2. Sincronización milimétrica con la voz (lead-in perceptual de 35ms y corte exacto al terminar la palabra).
        3. Frases completas equilibradas (4 a 6 palabras) con tipografía limpia y fade ultra-rápido.
        4. Sello numérico (#01-#05) dorado en la esquina superior izquierda.
        """
        output_ass_path.parent.mkdir(parents=True, exist_ok=True)
        
        # En 1080x1920: margin_v=480 ubica el texto exactamente en Y=1440 (libre de botones y nombre de página)
        font_size = 46 if self.width == 1080 else 50
        margin_v = 480 if self.width == 1080 else 120
...
"""
        ...
        for scene in scene_data:
            ...
            if not word_timings:
                ...
                events.append(f"Dialogue: 0,{s_time},{e_time},{style_name},,0,0,0,,{{\\fad(60,60)}}{formatted_text}")
                continue
            ...
            # 4. Sincronización acústica milimétrica (Frame-Perfect)
            for idx_c, chunk in enumerate(sentence_chunks):
                ...
                # Anticipación perceptual de 35ms para que el texto coincida exactamente con la vocal/consonante
                c_start = max(scene_start, chunk_raw_start - 0.035)
                
                # Corte limpio sin arrastre residual
                ...
                # Envolver en máximo 2 líneas equilibradas
                wrapped_clause = wrap_text_lines(clause_text, max_chars_per_line=32)
                
                # Transición ágil y limpia (50ms)
